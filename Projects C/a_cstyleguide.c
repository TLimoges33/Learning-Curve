...
//Similarly do companies typically adopt their own, company-wide conventions for style.
    ...
    //Line Length
//By convention the maximum length of a line of code is 80 characters long in C, 
//with that being historically grounded in standard-sized monitors on older computer terminals, 
//which could display 24 lines vertically and 80 characters horizontally. 
//Though modern technology has obsoleted the need to keep lines capped at 80 characters, 
//it is still a guideline that should be considered a “soft stop,” 
//and a line of 100 characters should really be the longest you write in C, 
//else readers will generally need to scroll. 
//If you need more than 100 characters, 
//it may be time to rethink either your variable names or your overall design!
...
In other languages, particularly JavaScript, it is significantly more difficult to constrain lines to a maximum length; there, your goal should instead be to break up lines (as via \n) in locations that maximize readability and clarity.
    ...
    //Comments
//Comments make code more readable, not only for others (e.g., your TF) but also for you, 
//especially when hours, days, weeks, months, or years pass between writing and reading your own code. 
//Commenting too little is bad. Commenting too much is bad. 
//Where’s the sweet spot? Commenting every few lines of code (i.e., interesting blocks) is a decent guideline. 
//Try to write comments that address one or both of these questions:
    ...
    //Within functions, use “inline comments” and keep them short (e.g., one line), 
      //else it becomes difficult to distinguish comments from code, even with syntax highlighting. 
      //Place the comment above the line(s) to which it applies. 
      //No need to write in full sentences, but do capitalize the comment’s first word (unless it’s the name of a function, variable, or the like), 
      //and do leave one space between the // and your comment’s first character, as in:
...
//else your code may not print or display properly on someone else’s computer, since \t renders differently in different editors. 
//(If using CS50 IDE, it’s fine to use Tab for indentation, rather than hitting your keyboard’s space bar repeatedly, since we’ve preconfigured it to convert \t to four spaces.)
...
//Variables
//Because CS50 uses C99, do not define all of your variables at the very top of your functions but, rather, when and where you actually need them. Moreover, scope your variables as tightly as possible. 
//For instance, if i is only needed for the sake of a loop, declare i within the loop itself:
...
//Though it’s fine to use variables like i, j, and k for iteration, most of your variables should be more specifically named. 
//If you’re summing some values, for instance, call your variable sum. 
//If your variable’s name warrants two words (e.g., is_ready), put an underscore between them, a convention popular in C though less so in other languages.
